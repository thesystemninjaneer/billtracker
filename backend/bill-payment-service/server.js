...
/**
 * @route POST /bills
 * @desc Add a new recurring bill entry
 * @access Private
 */
app.post('/bills', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      ...
      bill: { id: result.insertId, organizationId, billName, dueDay, typicalAmount, frequency, notes }
    });
  } catch (error) {
    ...
    res.status(500).json({ message: 'Server error adding bill entry.' });
  }
});
...
app.post('/payments', async (req, res) => {
  ...
  const user_id = req.user.id;
  const payment_status = (datePaid && amountPaid) ? 'paid' : 'pending'; // Auto-set status if paid info exists

  if (!organizationId || !dueDate || !amountDue) {
